...
def camera():
    ...
    while (i<=30):
        ret, img = cap.read()
        faces = face_c.detectMultiScale(img,1.05,5)

        for x,y,w,h in faces:

            face_img = img[y:y+h,x:x+w] 

            re_size = cv2.resize(face_img,(224,224))
            re_shaped=re_size.reshape(1, 224,224,3)/255
            predictions = model.predict(re_shaped)
            ...
            cv2.rectangle(img,(x,y),(x+w,y+h),Grid[1],2)
            cv2.rectangle(img,(x,y-40),(x+w,y),Grid[1],-1)
